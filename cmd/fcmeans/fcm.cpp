...
void FCM::compute_centers()
{
    ...
    MatrixXf t;
    t.resize(m_data->rows(), m_num_clustersC);
    if (m_data == nullptr || m_data->rows() == 0) {
        throw std::logic_error("ERROR: number of rows is zero");
        return;
    }
    for (i = 0; i < m_data->rows(); i++) { // compute (u^m) for each cluster for each point
        for (j = 0; j < m_num_clustersC; j++) {
            t(i, j) = pow((*m_membership)(i, j), m_m);
        }
    }
    for (j = 0; j < m_num_clustersC; j++) { // loop for each cluster
        for (k = 0; k < m_num_dimensionsN; k++) { // for each dimension
            numerator = 0.0;
            denominator = 0.0;
            ...
            (*m_centers)(j, k) = numerator / denominator;
        }
    }
}
...
double FCM::get_dist(long i, long k)
{
  ...
  */
    ...
    if (m_num_dimensionsN == 0) {
        throw std::logic_error("ERROR: number of dimensions should not be zero\n");
    }
    for (j = 0; j < m_num_dimensionsN; j++) {
        sqsum += pow(((*m_data)(k, j) - (*m_centers)(i, j)), 2);
    }
    return sqrt(sqsum);
}
...
void FCM::set_membership(MatrixXf* membership)
{
    ...
    if (m_membership->rows() == 0) {
        m_membership->resize(m_data->rows(), m_num_clustersC);
    }
}
...
{
    m_num_clustersC = num_clusters;
    if (m_centers) {
        delete m_centers;
    }
    m_centers = new MatrixXf;
    //                           c       n
    m_centers->resize(m_num_clustersC, m_num_dimensionsN);
}
